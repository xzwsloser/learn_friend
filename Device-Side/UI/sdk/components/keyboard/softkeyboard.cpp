...
#define PINYINFILEPATH  ":/keyboard_config/ChinesePinyin"
...
void SoftKeyboard::initNumberLetterBtn()
{
    ...
    for(int i=0;i<36;i++)//为10个数字，26个字母按键申请空间,连接信号与槽
    {
        ...
        QFont font = numberLetterBtn[i]->font();
...
void SoftKeyboard::initInputBufferArea()
{
    ...
    QFont font = inputTitleLabel->font();
    font.setPointSize(30);
    inputTitleLabel->setFont(font);
    //输入缓存内容编辑框
    inputContentEdit = new QLineEdit();
    inputContentEdit->setSizePolicy(QSizePolicy::Preferred,QSizePolicy::Preferred);
    font = inputContentEdit->font();
    font.setPointSize(30);
    inputContentEdit->setFont(font);
...
void SoftKeyboard::initFunctionAndCandidateArea()
{
    ...
    QFont font = introduceLabel->font();
    font.setPointSize(30);
    introduceLabel->setFont(font);
...
{
    //根据输入的内容自动改变文本区域大小
    int width = candidateLetter->fontMetrics().horizontalAdvance(text)+6;
